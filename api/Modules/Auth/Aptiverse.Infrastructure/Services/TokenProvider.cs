...
namespace Aptiverse.Infrastructure.Services
{
    public class TokenProvider : ITokenProvider
    {
            ...
            IEnumerable<string>? features = null)
        {
            ...
            // Each entitled feature gets its own `features` claim (JWTs
            // can't carry arrays natively — repeat the same name).
            if (features is not null)
            {
                foreach (var f in features.Distinct(StringComparer.OrdinalIgnoreCase))
                {
