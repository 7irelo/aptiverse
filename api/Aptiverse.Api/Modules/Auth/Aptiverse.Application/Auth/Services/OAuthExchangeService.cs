...
{
    /// <summary>
    /// One backend endpoint, two providers, all clients (web + mobile).
    /// Web flow: NextAuth completes OAuth dance and POSTs the ID token here.
    /// Mobile flow: native SDK completes OAuth and POSTs the ID token here.
    /// In both cases this service validates the token with the issuer
    /// (Google or Apple), upserts a User in Identity, and issues an
    /// Aptiverse JWT.
    /// </summary>
    public class OAuthExchangeService(
        ...
        ILogger<OAuthExchangeService> logger) : IOAuthExchangeService
    {
        private const string GoogleProvider = "google";
        private const string AppleProvider = "apple";
        private const string AppleIssuer = "https://appleid.apple.com";

        // Apple JWKS is fetched once per process, refreshed every 24h —
        // their keys rotate so we can't hard-code them.
        private static readonly Lazy<ConfigurationManager<OpenIdConnectConfiguration>> AppleConfigManager =
        ...
        public async Task<TokenDto<UserDto>> ExchangeAsync(OAuthExchangeDto request)
        {
            ...
            var (email, externalId, firstName, lastName) = request.Provider.ToLowerInvariant() switch
            {
                ...
                AppleProvider => await ValidateAppleAsync(request.IdToken),
            ...
            var token = await tokenProvider.GenerateJwtTokenAsync(user, roles);
        ...
        private async Task<(string Email, string ExternalId, string? FirstName, string? LastName)>
            ValidateAppleAsync(string idToken)
        {
            var appleClientId = config["Authentication:Apple:ClientId"];
            var appleConfig = await AppleConfigManager.Value.GetConfigurationAsync();

            var validationParameters = new TokenValidationParameters
            {
                ValidIssuer = AppleIssuer,
                ValidateIssuer = true,
                ValidateAudience = !string.IsNullOrWhiteSpace(appleClientId),
            ...
            try
            {
                ...
                var email = principal.FindFirstValue(ClaimTypes.Email)
                            ...
                            ?? throw new AuthenticationException("Apple token missing email claim");
                var sub = principal.FindFirstValue(ClaimTypes.NameIdentifier)
                          ?? principal.FindFirstValue("sub")
                          ?? throw new AuthenticationException("Apple token missing sub claim");

                // Apple only sends name on the *first* sign-in, and only if
                // the user shared it. Best effort.
                var firstName = principal.FindFirstValue("given_name");
                var lastName = principal.FindFirstValue("family_name");

                return (email, sub, firstName, lastName);
            }
            catch (SecurityTokenException ex)
            {
                logger.LogWarning(ex, "Invalid Apple ID token");
