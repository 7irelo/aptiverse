...
9. **The frontend's TypeScript types in `ui/src/lib/mockData.ts` are the contract for `/api/{module}/*` endpoints.** When you add a new field to a UI type, add it to the matching `Frontend{X}Dto` in the corresponding module's `Application/Frontend/Dtos/` and update the matching `{Module}Controller` in `Controllers/`. Field names are camelCase (`[JsonPropertyName(...)]`).
...
5. Add `Frontend/Dtos/FrontendDtos.cs` to the new Application project (folder name is historical; DTOs are still UI-shaped reads).
6. Add a `{Name}Controller.cs` to `Controllers/` with `[Route("api/{name-kebab-case}")]`. Namespace `Aptiverse.{Name}.Controllers`.
...
- Default to `[Authorize]` on controllers; opt into `[AllowAnonymous]` per-action when needed (e.g. FAQs).
...
| Add a `/api/{module}/X` endpoint | Matching field in `ui/src/lib/mockData.ts` (or use the API directly via TanStack hook) |
